...
class LaporanKehilanganController extends Controller
{
    ...
    {
        $query = Report::where('user_id', Auth::id())->with('transaction.book');

        // Search by book title
        if ($request->filled('search')) {
            $search = $request->search;
            $query->whereHas('transaction.book', fn($q) => $q->where('judul', 'like', "%$search%"));
        }

        // Filter by date (tanggal laporan dibuat)
